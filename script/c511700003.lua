...
function s.thfilter(c)
	return c:IsAbleToHand() and c:IsSetCard(SET_CENTURION) and c:IsLevelAbove(1)
end
...
function s.cfilter(c)
	return c:IsSetCard(SET_CENTURION) and c:IsAbleToRemoveAsCost() and not c:IsCode(id)
		and ((c:IsLocation(LOCATION_GRAVE|LOCATION_MZONE) and aux.SpElimFilter(c,true)) or c:IsLocation(LOCATION_HAND))
end

function s.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.cfilter,tp,LOCATION_MZONE|LOCATION_GRAVE,0,1,nil,e,tp) end
